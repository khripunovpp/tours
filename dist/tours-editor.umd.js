...
`;function Ne(r){const e=r.replace(/[.+^${}()|[\]\\]/g,"\\$&").replace(/\*\*/g,"\0").replace(/\*/g,"[^/]*").replace(/ /g,".*").replace(/\?/g,".");return new RegExp(`^${e}$`)}function j(r,e){if(!r)return!0;if(r.regex)try{return new RegExp(r.regex).test(e)}catch{return!1}if(r.glob)try{return Ne(r.glob).test(e)}catch{return!1}return!0}const V="tours:locationchange";let le=!1;function Oe(){if(!le){le=!0;for(const r of["pushState","replaceState"]){const e=history[r];history[r]=function(...n){const i=e.apply(this,n);return window.dispatchEvent(new Event(V)),i}}}}function ze(r){return Oe(),window.addEventListener("popstate",r),window.addEventListener("hashchange",r),window.addEventListener(V,r),()=>{window.removeEventListener("popstate",r),window.removeEventListener("hashchange",r),window.removeEventListener(V,r)}}const ce="tours:progress";function Ie(r,e){r.set(ce,JSON.stringify(e))}function Re(r){r.remove(ce)}function Me(r,e={}){const t=M("player"),n=e.state;let i=null,o=null,s=null,l=null,p=!1,d=0,u=null;const h=r.display?.padding??D,k=r.display?.radius??B,T=r.display?.cardRadius??U,w=r.display?.offset??F;function f(c){return P(c.selectors)}function g(c){return j(c.pageUrl,window.location.href)}function m(){n&&Ie(n,{tourId:r.id,index:d})}function he(){if(i)return;i=document.createElement("div"),i.setAttribute("data-tours-player",""),o=i.attachShadow({mode:"open"});const c=document.createElement("style");c.textContent=xe+de,o.appendChild(c);const b=document.createElement("div");b.className="tours-backdrop",o.appendChild(b),s=document.createElement("div"),s.className="tours-spotlight",s.style.borderRadius=`${k}px`,o.appendChild(s),document.body.appendChild(i)}function fe(c){s&&(s.style.display="block",s.style.left=`${c.left-h}px`,s.style.top=`${c.top-h}px`,s.style.width=`${c.width+h*2}px`,s.style.height=`${c.height+h*2}px`)}function ge(c,b){if(!l)return;const x={top:c.top-h,left:c.left-h,right:c.right+h,bottom:c.bottom+h,width:c.width+h*2,height:c.height+h*2},{top:K,left:He}=oe({target:x,card:{width:l.offsetWidth,height:l.offsetHeight},side:b.placement??"bottom",align:b.align??"center",offset:w,alignOffset:r.display?.alignOffset??0,viewport:{width:window.innerWidth,height:window.innerHeight}});l.style.left=`${He}px`,l.style.top=`${K}px`}function Fe(c){const b=r.steps.length;l&&l.remove(),l=ae({contentText:c.content.default,progress:`Step ${d+1} of ${b}`,showClose:!0,onClose:S,radius:T,back:{label:c.backLabel??"Back",disabled:d===0,onClick:G},next:{label:c.nextLabel??(d===b-1?"Done":"Next"),primary:!0,onClick:q}}),o?.appendChild(l)}function E(){if(!p)return;const c=r.steps[d];if(!c){S();return}t.log("render step",d,c.id);const b=f(c);if(!b){t.log(`step "${c.id}" target not found yet — waiting`,c.selectors),Ce(c.selectors,{timeout:4e3}).then(K=>{!p||r.steps[d]!==c||(K?E():(t.warn(`step "${c.id}" skipped: no element for selectors`,c.selectors),d<r.steps.length-1?(d+=1,E()):S()))});return}he(),b.scrollIntoView({behavior:"smooth",block:"center",inline:"center"}),Fe(c);const x=b.getBoundingClientRect();fe(x),ge(x,c)}function be(c){p&&(c.key==="Escape"?(c.preventDefault(),S()):c.key==="ArrowRight"?q():c.key==="ArrowLeft"&&G())}function z(){if(!p)return;const c=r.steps[d];if(!c)return;const b=f(c);if(!b)return;const x=b.getBoundingClientRect();fe(x),ge(x,c)}function je(c=0){p||r.steps.length!==0&&(p=!0,d=Math.max(0,Math.min(c,r.steps.length-1)),t.log("start",r.id,`at ${d}/${r.steps.length}`),he(),window.addEventListener("keydown",be,!0),window.addEventListener("resize",z,!0),window.addEventListener("scroll",z,!0),m(),E())}function Ve(){s&&(s.style.display="none"),l&&(l.remove(),l=null)}function me(){Ve(),!u&&(u=ze(()=>{if(!p){u?.(),u=null;return}const c=r.steps[d];c&&g(c)&&(u?.(),u=null,E())}))}function ve(){u&&(u(),u=null),p&&(p=!1,window.removeEventListener("keydown",be,!0),window.removeEventListener("resize",z,!0),window.removeEventListener("scroll",z,!0),i&&i.parentNode&&i.parentNode.removeChild(i),i=null,o=null,s=null,l=null)}function S(){t.log("stop"),ve(),n&&Re(n)}function q(){if(!p)return;const c=d+1,b=r.steps[c];if(!b){S();return}if(g(b)){d=c,m(),E();return}d=c,m();const x=r.steps[d-1]?.action;if(x&&x.type==="navigate"&&x.url){t.log("page transition (navigate) → resume at",d),ve(),window.location.assign(x.url);return}t.log("page transition (wait) → resume at",d),me()}function G(){if(!p)return;const c=r.steps[d-1];if(c){if(g(c)){d-=1,m(),E();return}d-=1,m(),t.log("page transition back → resume at",d),me(),window.history.back()}}return{start:je,stop:S,next:q,prev:G}}const De=`
:host {
  all: initial;
  --e-bg: #ffffff;
  --e-fg: #1f2733;
