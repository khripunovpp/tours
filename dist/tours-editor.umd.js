...
`;function ue(r){const e=O("player");let t=null,n=null,i=null,o=null,s=!1,d=0;const l=r.display?.padding??I,u=r.display?.radius??R,h=r.display?.cardRadius??D,g=r.display?.offset??M;function k(c){for(const f of c.selectors)try{const w=document.querySelector(f);if(w)return w}catch{}return null}function S(){if(t)return;t=document.createElement("div"),t.setAttribute("data-tours-player",""),n=t.attachShadow({mode:"open"});const c=document.createElement("style");c.textContent=oe+te,n.appendChild(c);const f=document.createElement("div");f.className="tours-backdrop",n.appendChild(f),i=document.createElement("div"),i.className="tours-spotlight",i.style.borderRadius=`${u}px`,n.appendChild(i),document.body.appendChild(t)}function x(c){i&&(i.style.display="block",i.style.left=`${c.left-l}px`,i.style.top=`${c.top-l}px`,i.style.width=`${c.width+l*2}px`,i.style.height=`${c.height+l*2}px`)}function p(c,f){if(!o)return;const w={top:c.top-l,left:c.left-l,right:c.right+l,bottom:c.bottom+l,width:c.width+l*2,height:c.height+l*2},{top:ve,left:me}=Z({target:w,card:{width:o.offsetWidth,height:o.offsetHeight},side:f.placement??"bottom",align:f.align??"center",offset:g,alignOffset:r.display?.alignOffset??0,viewport:{width:window.innerWidth,height:window.innerHeight}});o.style.left=`${me}px`,o.style.top=`${ve}px`}function b(c){const f=r.steps.length;o&&o.remove(),o=ee({contentText:c.content.default,progress:`Step ${d+1} of ${f}`,showClose:!0,onClose:C,radius:h,back:{label:c.backLabel??"Back",disabled:d===0,onClick:H},next:{label:c.nextLabel??(d===f-1?"Done":"Next"),primary:!0,onClick:V}}),n?.appendChild(o)}function v(){if(!s)return;const c=r.steps[d];if(!c){C();return}e.log("render step",d,c.id);const f=k(c);if(!f){e.warn(`step "${c.id}" skipped: no element for selectors`,c.selectors),d<r.steps.length-1?(d+=1,v()):C();return}S(),f.scrollIntoView({behavior:"smooth",block:"center",inline:"center"}),b(c);const w=f.getBoundingClientRect();x(w),p(w,c)}function ie(c){s&&(c.key==="Escape"?(c.preventDefault(),C()):c.key==="ArrowRight"?V():c.key==="ArrowLeft"&&H())}function N(){if(!s)return;const c=r.steps[d];if(!c)return;const f=k(c);if(!f)return;const w=f.getBoundingClientRect();x(w),p(w,c)}function be(){s||r.steps.length!==0&&(s=!0,d=0,e.log("start",r.id,`${r.steps.length} steps`),S(),window.addEventListener("keydown",ie,!0),window.addEventListener("resize",N,!0),window.addEventListener("scroll",N,!0),v())}function C(){s&&(s=!1,e.log("stop"),window.removeEventListener("keydown",ie,!0),window.removeEventListener("resize",N,!0),window.removeEventListener("scroll",N,!0),t&&t.parentNode&&t.parentNode.removeChild(t),t=null,n=null,i=null,o=null)}function V(){if(s){if(d>=r.steps.length-1){C();return}d+=1,v()}}function H(){s&&(d<=0||(d-=1,v()))}return{start:be,stop:C,next:V,prev:H}}const he=`
:host {
  all: initial;
  --e-bg: #ffffff;
  --e-fg: #1f2733;
