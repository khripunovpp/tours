...
`;function B(r){return JSON.stringify(r)}function ke(r){return/^[a-zA-Z][\w-]*$/.test(r)&&r.length<=30&&!/\d{2,}/.test(r)&&!/^(css-|sc-|jsx-|emotion-|_|is-|has-)/.test(r)}function te(r){const e=[];let t=r;for(;t&&t!==document.body&&t.nodeType===1;){const n=t.tagName.toLowerCase(),i=t.parentElement;if(!i){e.unshift(n);break}const o=Array.from(i.children).filter(s=>s.tagName===t.tagName);e.unshift(o.length>1?`${n}:nth-of-type(${o.indexOf(t)+1})`:n),t=i}return`body > ${e.join(" > ")}`}function Se(r){let e=r.parentElement;for(;e&&e!==document.body&&!e.id;)e=e.parentElement;if(!e||!e.id)return null;const t=[];let n=r;for(;n&&n!==e;){const i=n.tagName.toLowerCase(),o=n.parentElement;if(!o)return null;const s=Array.from(o.children).filter(l=>l.tagName===n.tagName);t.unshift(s.length>1?`${i}:nth-of-type(${s.indexOf(n)+1})`:i),n=o}return`#${CSS.escape(e.id)} > ${t.join(" > ")}`}const Ee=["data-testid","data-test","data-test-id","data-cy","data-qa","data-id","data-name"];function Ce(r){const e=[],t=new Set,n=r.tagName.toLowerCase(),i=d=>{if(!(!d||t.has(d)))try{document.querySelector(d)===r&&(t.add(d),e.push(d))}catch{}};r.id&&i(`#${CSS.escape(r.id)}`);for(const d of Ee){const u=r.getAttribute(d);u&&i(`${n}[${d}=${B(u)}]`)}const o=r.getAttribute("name");o&&i(`${n}[name=${B(o)}]`);const s=r.getAttribute("aria-label");s&&i(`[aria-label=${B(s)}]`);const l=Array.from(r.classList).filter(ke);l.length&&i(`${n}.${l.map(d=>CSS.escape(d)).join(".")}`);for(const d of l)i(`${n}.${CSS.escape(d)}`);i(Se(r)),i(te(r));const p=(r.textContent??"").replace(/\s+/g," ").trim();if(p&&p.length<=50&&/^(a|button|summary|label|h[1-6])$/.test(n)){const d=`text=${p}`;t.has(d)||(t.add(d),e.push(d))}return e.length===0&&e.push(te(r)),e}const $e="a, button, summary, label, h1, h2, h3, h4, h5, h6";function Le(r,e){if(r.startsWith("text=")){const t=r.slice(5).trim();for(const n of Array.from(e.querySelectorAll($e)))if((n.textContent??"").replace(/\s+/g," ").trim()===t)return n;return null}try{return e.querySelector(r)}catch{return null}}function O(r,e=document){for(const t of r){const n=Le(t,e);if(n)return n}return null}function Te(r,e={}){const t=e.root??document,n=O(r,t);return n?Promise.resolve(n):new Promise(i=>{let o=!1,s;const l=u=>{o||(o=!0,p.disconnect(),s&&clearTimeout(s),i(u))},p=new MutationObserver(()=>{const u=O(r,t);u&&l(u)});p.observe(document.documentElement,{childList:!0,subtree:!0,attributes:!0});const d=e.timeout??4e3;d>0&&Number.isFinite(d)&&(s=setTimeout(()=>l(null),d))})}let T=null;function F(){if(T!==null)return T;try{T=new URLSearchParams(window.location.search).has("use_logs")}catch{T=!1}return T}function U(r){const e=`[tours:${r}]`;return{log:(...t)=>{F()&&console.log(e,...t)},warn:(...t)=>{F()&&console.warn(e,...t)},error:(...t)=>{F()&&console.error(e,...t)}}}function Pe(r,e={}){const t=U("picker");let n=null,i=null,o=null,s=!1;function l(f){if(f===n)return!0;for(const v of e.ignore??[])if(v&&v.contains(f))return!0;return!1}function p(){if(n)return;n=document.createElement("div"),n.setAttribute("data-tours-picker",""),i=n.attachShadow({mode:"open"});const f=document.createElement("style");f.textContent=L,i.appendChild(f),o=document.createElement("div"),o.className="tours-picker-overlay",o.style.display="none",i.appendChild(o);const v=document.createElement("div");v.className="tours-picker-hint",v.textContent="Hover and click an element • Esc to cancel",i.appendChild(v),document.body.appendChild(n)}function d(f,v){const x=document.elementFromPoint(f,v);return!x||l(x)?null:x}function u(f){if(!s||!o)return;const v=d(f.clientX,f.clientY);if(!v){o.style.display="none";return}const x=v.getBoundingClientRect();o.style.display="block",o.style.left=`${x.left}px`,o.style.top=`${x.top}px`,o.style.width=`${x.width}px`,o.style.height=`${x.height}px`}function h(f){if(!s)return;const v=d(f.clientX,f.clientY);if(f.preventDefault(),f.stopPropagation(),!v)return;const x=Ce(v);t.log("picked",x),_(),r(x)}function b(f){f.key==="Escape"&&(f.preventDefault(),_())}function P(){s||(s=!0,t.log("start"),p(),document.addEventListener("mousemove",u,!0),document.addEventListener("click",h,!0),document.addEventListener("keydown",b,!0))}function _(){s&&(s=!1,document.removeEventListener("mousemove",u,!0),document.removeEventListener("click",h,!0),document.removeEventListener("keydown",b,!0),n&&n.parentNode&&n.parentNode.removeChild(n),n=null,i=null,o=null)}return{start:P,stop:_}}const V=6,j=6,H=10,W=12,Ae=1;function k(r){return typeof r=="object"&&r!==null&&!Array.isArray(r)}function ne(r){return k(r)&&typeof r.default=="string"}const re=["top","bottom","left","right","auto"],ie=["start","center","end"],oe=["mobile","tablet","desktop"],se=["click","input","navigate","none"];function ae(r,e,t){if(!k(r)){t.push(`${e} must be an object`);return}const n=typeof r.glob=="string"&&r.glob.length>0,i=typeof r.regex=="string"&&r.regex.length>0;if(!n&&!i&&t.push(`${e} must have a non-empty "glob" or "regex"`),i)try{new RegExp(r.regex)}catch{t.push(`${e}.regex is not a valid regular expression`)}}function de(r,e,t){if(!k(r)){t.push(`${e} must be an object`);return}r.url!==void 0&&ae(r.url,`${e}.url`,t),r.role!==void 0&&typeof r.role!="string"&&t.push(`${e}.role must be a string`),r.firstVisitOnly!==void 0&&typeof r.firstVisitOnly!="boolean"&&t.push(`${e}.firstVisitOnly must be a boolean`),r.device!==void 0&&!oe.includes(r.device)&&t.push(`${e}.device must be one of ${oe.join("|")}`),r.unlessSeen!==void 0&&typeof r.unlessSeen!="boolean"&&t.push(`${e}.unlessSeen must be a boolean`),r.maxShows!==void 0&&(typeof r.maxShows!="number"||r.maxShows<0)&&t.push(`${e}.maxShows must be a non-negative number`)}function Oe(r,e,t){if(!k(r)){t.push(`${e} must be an object`);return}se.includes(r.type)||t.push(`${e}.type must be one of ${se.join("|")}`),r.url!==void 0&&typeof r.url!="string"&&t.push(`${e}.url must be a string`),r.value!==void 0&&typeof r.value!="string"&&t.push(`${e}.value must be a string`)}function Ne(r){const e=[];if(!k(r))return{ok:!1,errors:["tour must be an object"]};if((typeof r.id!="string"||r.id.length===0)&&e.push("tour.id must be a non-empty string"),typeof r.schemaVersion!="number"&&e.push("tour.schemaVersion must be a number"),ne(r.title)||e.push('tour.title must be a localized text with a string "default"'),Array.isArray(r.steps)?r.steps.length===0?e.push("tour.steps must contain at least one step"):r.steps.forEach((t,n)=>{if(!k(t)){e.push(`steps[${n}] must be an object`);return}(typeof t.id!="string"||t.id.length===0)&&e.push(`steps[${n}].id must be a non-empty string`),(!Array.isArray(t.selectors)||t.selectors.length===0||!t.selectors.every(i=>typeof i=="string"&&i.length>0))&&e.push(`steps[${n}].selectors must be a non-empty array of non-empty strings`),ne(t.content)||e.push(`steps[${n}].content must be a localized text with a string "default"`),t.placement!==void 0&&!re.includes(t.placement)&&e.push(`steps[${n}].placement must be one of ${re.join("|")}`),t.align!==void 0&&!ie.includes(t.align)&&e.push(`steps[${n}].align must be one of ${ie.join("|")}`),t.backLabel!==void 0&&typeof t.backLabel!="string"&&e.push(`steps[${n}].backLabel must be a string`),t.nextLabel!==void 0&&typeof t.nextLabel!="string"&&e.push(`steps[${n}].nextLabel must be a string`),t.pageUrl!==void 0&&ae(t.pageUrl,`steps[${n}].pageUrl`,e),t.condition!==void 0&&de(t.condition,`steps[${n}].condition`,e),t.action!==void 0&&Oe(t.action,`steps[${n}].action`,e)}):e.push("tour.steps must be an array"),r.trigger!==void 0){const t=r.trigger,n=["manual","load","selector","timer"];!k(t)||typeof t.type!="string"||!n.includes(t.type)?e.push(`tour.trigger.type must be one of ${n.join("|")}`):t.type==="selector"&&(typeof t.selector!="string"||t.selector.length===0)?e.push("tour.trigger.selector must be a non-empty string"):t.type==="timer"&&(typeof t.delay!="number"||t.delay<0)&&e.push("tour.trigger.delay must be a non-negative number")}if(r.audience!==void 0&&!["all","auth","guest"].includes(r.audience)&&e.push("tour.audience must be one of all|auth|guest"),r.display!==void 0)if(!k(r.display))e.push("tour.display must be an object");else for(const t of["padding","radius","cardRadius","offset","alignOffset"]){const n=r.display[t];n!==void 0&&(typeof n!="number"||n<0)&&e.push(`tour.display.${t} must be a non-negative number`)}return r.rules!==void 0&&(Array.isArray(r.rules)?r.rules.forEach((t,n)=>{if(!k(t)){e.push(`rules[${n}] must be an object`);return}t.tourId!==void 0&&typeof t.tourId!="string"&&e.push(`rules[${n}].tourId must be a string`),t.when===void 0?e.push(`rules[${n}].when is required`):de(t.when,`rules[${n}].when`,e)}):e.push("tour.rules must be an array")),e.length>0?{ok:!1,errors:e}:{ok:!0,tour:r}}function ze(r,e,t){const n={top:r.top,bottom:t.height-r.bottom,left:r.left,right:t.width-r.right},i={top:e.height,bottom:e.height,left:e.width,right:e.width},o=["bottom","top","right","left"],s=o.find(l=>n[l]>=i[l]+8);return s||o.reduce((l,p)=>n[p]>n[l]?p:l,o[0])}function le(r){const{target:e,card:t,offset:n,viewport:i}=r,o=r.side==="auto",s=o?ze(e,t,i):r.side,l=o?"center":r.align,p=r.alignOffset??0,d=l==="start"?p:l==="end"?-p:0;let u=0,h=0;return s==="top"||s==="bottom"?(u=s==="top"?e.top-t.height-n:e.bottom+n,h=l==="start"?e.left:l==="end"?e.right-t.width:e.left+e.width/2-t.width/2,h+=d):(h=s==="left"?e.left-t.width-n:e.right+n,u=l==="start"?e.top:l==="end"?e.bottom-t.height:e.top+e.height/2-t.height/2,u+=d),h=Math.max(8,Math.min(h,i.width-t.width-8)),u=Math.max(8,Math.min(u,i.height-t.height-8)),{top:u,left:h}}function ce(r){const e=document.createElement("button");return e.type="button",e.className=`tours-card__btn${r.primary?" tours-card__btn--primary":""}${r.disabled?" tours-card__btn--disabled":""}`,e.textContent=r.label,!r.disabled&&r.onClick&&e.addEventListener("click",r.onClick),e}function pe(r){const e=document.createElement("div");if(e.className=`tours-card${r.ghost?" tours-card--ghost":""}`,r.radius!=null&&(e.style.borderRadius=`${r.radius}px`),r.showClose){const n=document.createElement("button");n.className="tours-card__close",n.type="button",n.textContent="×",n.setAttribute("aria-label","Close"),r.onClose&&n.addEventListener("click",r.onClose),e.appendChild(n)}const t=document.createElement("div");if(t.className="tours-card__content",r.contentHtml!=null?t.innerHTML=r.contentHtml:t.textContent=r.contentText??"",e.appendChild(t),r.back||r.next||r.progress){const n=document.createElement("div");if(n.className="tours-card__footer",r.back&&n.appendChild(ce(r.back)),r.progress){const i=document.createElement("span");i.className="tours-card__progress",i.textContent=r.progress,n.appendChild(i)}r.next&&n.appendChild(ce(r.next)),e.appendChild(n)}return e}const ue=`
.tours-card {
  position: fixed;
  z-index: 2147483001;
  box-sizing: border-box;
...
`;function Re(r){const e=r.replace(/[.+^${}()|[\]\\]/g,"\\$&").replace(/\*\*/g,"\0").replace(/\*/g,"[^/]*").replace(/ /g,".*").replace(/\?/g,".");return new RegExp(`^${e}$`)}function J(r,e){if(!r)return!0;if(r.regex)try{return new RegExp(r.regex).test(e)}catch{return!1}if(r.glob)try{return Re(r.glob).test(e)}catch{return!1}return!0}const Y="tours:locationchange";let he=!1;function Ie(){if(!he){he=!0;for(const r of["pushState","replaceState"]){const e=history[r];history[r]=function(...n){const i=e.apply(this,n);return window.dispatchEvent(new Event(Y)),i}}}}function Me(r){return Ie(),window.addEventListener("popstate",r),window.addEventListener("hashchange",r),window.addEventListener(Y,r),()=>{window.removeEventListener("popstate",r),window.removeEventListener("hashchange",r),window.removeEventListener(Y,r)}}const fe="tours:progress";function De(r,e){r.set(fe,JSON.stringify(e))}function Be(r){r.remove(fe)}function Fe(r,e={}){const t=U("player"),n=e.state;let i=null,o=null,s=null,l=null,p=!1,d=0,u=0,h=null;const b=r.display?.padding??V,P=r.display?.radius??j,_=r.display?.cardRadius??H,f=r.display?.offset??W;function v(c){return O(c.selectors)}function x(c){return J(c.pageUrl,window.location.href)}function A(){n&&De(n,{tourId:r.id,index:d})}function me(){if(i)return;i=document.createElement("div"),i.setAttribute("data-tours-player",""),o=i.attachShadow({mode:"open"});const c=document.createElement("style");c.textContent=_e+ue,o.appendChild(c);const g=document.createElement("div");g.className="tours-backdrop",g.addEventListener("click",m=>{const $=r.steps[d],M=$?v($):null;if(M){const D=M.getBoundingClientRect();if(m.clientX>=D.left-b&&m.clientX<=D.right+b&&m.clientY>=D.top-b&&m.clientY<=D.bottom+b)return}S()}),o.appendChild(g),s=document.createElement("div"),s.className="tours-spotlight",s.style.borderRadius=`${P}px`,o.appendChild(s),document.body.appendChild(i)}function ve(c,g=!1){s&&(s.style.transitionDuration=g?"0ms":"",s.style.display="block",s.style.left=`${c.left-b}px`,s.style.top=`${c.top-b}px`,s.style.width=`${c.width+b*2}px`,s.style.height=`${c.height+b*2}px`)}function xe(c,g){if(!l)return;const m={top:c.top-b,left:c.left-b,right:c.right+b,bottom:c.bottom+b,width:c.width+b*2,height:c.height+b*2},{top:$,left:M}=le({target:m,card:{width:l.offsetWidth,height:l.offsetHeight},side:g.placement??"bottom",align:g.align??"center",offset:f,alignOffset:r.display?.alignOffset??0,viewport:{width:window.innerWidth,height:window.innerHeight}});l.style.left=`${M}px`,l.style.top=`${$}px`}function Ye(c){const g=Math.max(1,r.steps.length-u),m=Math.max(1,Math.min(d+1-u,g));l&&l.remove(),l=pe({contentText:c.content.default,progress:`Step ${m} of ${g}`,showClose:!0,onClose:S,radius:_,back:{label:c.backLabel??"Back",disabled:d===0,onClick:ee},next:{label:c.nextLabel??(d===g-1?"Done":"Next"),primary:!0,onClick:Q}}),o?.appendChild(l)}function C(){if(!p)return;const c=r.steps[d];if(!c){S();return}t.log("render step",d,c.id);const g=v(c);if(!g){t.log(`step "${c.id}" target not found yet — waiting`,c.selectors),Te(c.selectors,{timeout:4e3}).then($=>{!p||r.steps[d]!==c||($?C():(t.warn(`step "${c.id}" skipped: no element for selectors`,c.selectors),u+=1,d<r.steps.length-1?(d+=1,C()):S()))});return}me(),g.scrollIntoView({behavior:"smooth",block:"center",inline:"center"}),Ye(c);const m=g.getBoundingClientRect();ve(m),xe(m,c)}function ye(c){p&&(c.key==="Escape"?(c.preventDefault(),S()):c.key==="ArrowRight"?Q():c.key==="ArrowLeft"&&ee())}function I(){if(!p)return;const c=r.steps[d];if(!c)return;const g=v(c);if(!g)return;const m=g.getBoundingClientRect();ve(m,!0),xe(m,c)}function qe(c=0){p||r.steps.length!==0&&(p=!0,d=Math.max(0,Math.min(c,r.steps.length-1)),u=0,t.log("start",r.id,`at ${d}/${r.steps.length}`),me(),window.addEventListener("keydown",ye,!0),window.addEventListener("resize",I,!0),window.addEventListener("scroll",I,!0),A(),C())}function Xe(){s&&(s.style.display="none"),l&&(l.remove(),l=null)}function Z(){Xe(),!h&&(h=Me(()=>{if(!p){h?.(),h=null;return}const c=r.steps[d];c&&x(c)&&(h?.(),h=null,C())}))}function we(){h&&(h(),h=null),p&&(p=!1,window.removeEventListener("keydown",ye,!0),window.removeEventListener("resize",I,!0),window.removeEventListener("scroll",I,!0),i&&i.parentNode&&i.parentNode.removeChild(i),i=null,o=null,s=null,l=null)}function S(){t.log("stop"),we(),n&&Be(n)}function Q(){if(!p)return;const c=d+1,g=r.steps[c];if(!g){S();return}if(x(g)){d=c,A(),C();return}d=c,A();const m=r.steps[d-1]?.action;if(m&&m.type==="navigate"&&m.url){m.url.startsWith("#")?(t.log("page transition (hash navigate) → resume at",d),Z(),window.location.hash=m.url):(t.log("page transition (navigate) → resume at",d),we(),window.location.assign(m.url));return}t.log("page transition (wait) → resume at",d),Z()}function ee(){if(!p)return;const c=r.steps[d-1];if(c){if(x(c)){d-=1,A(),C();return}d-=1,A(),t.log("page transition back → resume at",d),Z(),window.history.back()}}return{start:qe,stop:S,next:Q,prev:ee}}const Ue=`
:host {
  all: initial;
  --e-bg: #ffffff;
  --e-fg: #1f2733;
