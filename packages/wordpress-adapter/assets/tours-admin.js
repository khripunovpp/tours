...
`;function Fe(r){const e=r.replace(/[.+^${}()|[\]\\]/g,"\\$&").replace(/\*\*/g,"\0").replace(/\*/g,"[^/]*").replace(/ /g,".*").replace(/\?/g,".");return new RegExp(`^${e}$`)}function U(r,e){if(!r)return!0;if(r.regex)try{return new RegExp(r.regex).test(e)}catch{return!1}if(r.glob)try{return Fe(r.glob).test(e)}catch{return!1}return!0}function ge(r){if(!r||!r.glob)return null;const e=r.glob.replace(/\*+/g,"");return/^https?:\/\//i.test(e)||e.startsWith("#")||e.startsWith("/")?e:null}const X="tours:locationchange";let be=!1;function Ue(){if(!be){be=!0;for(const r of["pushState","replaceState"]){const e=history[r];history[r]=function(...n){const i=e.apply(this,n);return window.dispatchEvent(new Event(X)),i}}}}function je(r){return Ue(),window.addEventListener("popstate",r),window.addEventListener("hashchange",r),window.addEventListener(X,r),()=>{window.removeEventListener("popstate",r),window.removeEventListener("hashchange",r),window.removeEventListener(X,r)}}const me="tours:progress";function Ve(r,e){r.set(me,JSON.stringify(e))}function He(r){r.remove(me)}const We="[data-tours-editor]";function Je(){return typeof document<"u"&&document.querySelector(We)!==null}function qe(r,e={}){const t=Y("player"),n=e.state;let i=null,o=null,s=null,l=null,c=!1,d=0,u=0,f=null;const b=r.display?.padding??z,R=r.display?.radius??D,S=r.display?.cardRadius??B,h=r.display?.offset??F;function m(p){return M(p.selectors)}function x(p){return U(p.pageUrl,window.location.href)}function I(){n&&Ve(n,{tourId:r.id,index:d})}function ke(){if(i)return;i=document.createElement("div"),i.setAttribute("data-tours-player",""),o=i.attachShadow({mode:"open"});const p=document.createElement("style");p.textContent=Ce+fe,o.appendChild(p);const g=document.createElement("div");g.className="tours-backdrop",g.addEventListener("click",v=>{const w=r.steps[d],E=w?m(w):null;if(E){const L=E.getBoundingClientRect();if(v.clientX>=L.left-b&&v.clientX<=L.right+b&&v.clientY>=L.top-b&&v.clientY<=L.bottom+b)return}C()}),o.appendChild(g),s=document.createElement("div"),s.className="tours-spotlight",s.style.borderRadius=`${R}px`,o.appendChild(s),document.body.appendChild(i)}function _e(p,g=!1){s&&(s.style.transitionDuration=g?"0ms":"",s.style.display="block",s.style.left=`${p.left-b}px`,s.style.top=`${p.top-b}px`,s.style.width=`${p.width+b*2}px`,s.style.height=`${p.height+b*2}px`)}function Se(p,g){if(!l)return;const v={top:p.top-b,left:p.left-b,right:p.right+b,bottom:p.bottom+b,width:p.width+b*2,height:p.height+b*2},{top:w,left:E}=pe({target:v,card:{width:l.offsetWidth,height:l.offsetHeight},side:g.placement??"bottom",align:g.align??"center",offset:h,alignOffset:r.display?.alignOffset??0,viewport:{width:window.innerWidth,height:window.innerHeight}});l.style.left=`${E}px`,l.style.top=`${w}px`}function nt(p){const g=Math.max(1,r.steps.length-u),v=Math.max(1,Math.min(d+1-u,g));l&&l.remove(),l=he({contentText:p.content.default,progress:`Step ${v} of ${g}`,showClose:!0,onClose:C,radius:S,back:{label:p.backLabel??"Back",disabled:d===0,onClick:te},next:{label:p.nextLabel??(d===g-1?"Done":"Next"),primary:!0,onClick:ee}}),o?.appendChild(l)}function P(){if(!c)return;const p=r.steps[d];if(!p){C();return}t.log("render step",d,p.id);const g=m(p);if(!g){t.log(`step "${p.id}" target not found yet — waiting`,p.selectors),Re(p.selectors,{timeout:4e3}).then(w=>{!c||r.steps[d]!==p||(w?P():(t.warn(`step "${p.id}" skipped: no element for selectors`,p.selectors),u+=1,d<r.steps.length-1?(d+=1,P()):C()))});return}ke(),g.scrollIntoView({behavior:"smooth",block:"center",inline:"center"}),nt(p);const v=g.getBoundingClientRect();_e(v),Se(v,p)}function Ee(p){c&&(p.key==="Escape"?(p.preventDefault(),C()):p.key==="ArrowRight"?ee():p.key==="ArrowLeft"&&te())}function H(){if(!c)return;const p=r.steps[d];if(!p)return;const g=m(p);if(!g)return;const v=g.getBoundingClientRect();_e(v,!0),Se(v,p)}function rt(p=0){if(!c&&r.steps.length!==0){if(!e.allowWhileEditing&&Je()){t.log(`start suppressed for "${r.id}" — the builder is mounted`);return}c=!0,d=Math.max(0,Math.min(p,r.steps.length-1)),u=0,t.log("start",r.id,`at ${d}/${r.steps.length}`),ke(),window.addEventListener("keydown",Ee,!0),window.addEventListener("resize",H,!0),window.addEventListener("scroll",H,!0),I(),P()}}function it(){s&&(s.style.display="none"),l&&(l.remove(),l=null)}function W(){it(),!f&&(f=je(()=>{if(!c){f?.(),f=null;return}const p=r.steps[d];p&&x(p)&&(f?.(),f=null,P())}))}function $e(){f&&(f(),f=null),c&&(c=!1,window.removeEventListener("keydown",Ee,!0),window.removeEventListener("resize",H,!0),window.removeEventListener("scroll",H,!0),i&&i.parentNode&&i.parentNode.removeChild(i),i=null,o=null,s=null,l=null)}function C(){t.log("stop"),$e(),n&&He(n)}function ee(){if(!c)return;const p=d+1,g=r.steps[p];if(!g){C();return}if(x(g)){d=p,I(),P();return}d=p,I();const v=L=>{$e(),e.onNavigate?e.onNavigate(L,g.id):window.location.assign(L)},w=r.steps[d-1]?.action;if(w&&w.type==="navigate"&&w.url){w.url.startsWith("#")?(t.log("page transition (hash navigate) → resume at",d),W(),window.location.hash=w.url):(t.log("page transition (navigate) → resume at",d),v(w.url));return}const E=ge(g.pageUrl);if(E){E.startsWith("#")?(t.log("page transition (derived hash) → resume at",d),W(),window.location.hash=E):(t.log("page transition (derived navigate) → resume at",d,E),v(E));return}t.log("page transition (wait) → resume at",d),W()}function te(){if(!c)return;const p=r.steps[d-1];if(p){if(x(p)){d-=1,I(),P();return}d-=1,I(),t.log("page transition back → resume at",d),W(),window.history.back()}}return{start:rt,stop:C,next:ee,prev:te}}const Ye=`
:host {
  all: initial;
  --e-bg: #ffffff;
  --e-fg: #1f2733;
